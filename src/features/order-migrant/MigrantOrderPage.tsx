      ...
      <div id="menu-section" className="relative w-full">
        ...
        {!packageSize && (
          <div className="bg-[#1B4332] relative z-10">
            <div className="mx-auto max-w-2xl px-5 py-10 md:py-14 text-center">
              <p className="text-[#E8927C] font-semibold text-xs uppercase tracking-widest mb-3">
                {tm('stepPackage')}
              </p>
              <h2 className="font-heading font-black text-3xl md:text-4xl uppercase tracking-tight text-white mb-8">
                {tm('packageTitle')}
              </h2>

              {availableWindows.length === 0 ? (
                <p className="text-white/60 text-base mt-4">{tm('noWindowAvailable')}</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                  {/* 3-day package */}
                  <button
                    onClick={() => handlePackageSelect(3)}
                    className="relative flex flex-col items-center gap-2 rounded-2xl bg-white/10 hover:bg-white/20 border-2 border-white/20 hover:border-[#E8927C] p-6 text-white transition-all active:scale-95"
                  >
                    <Package className="h-8 w-8 text-[#E8927C] mb-1" />
                    <span className="font-heading font-black text-2xl uppercase">
                      {tm('package3days')}
                    </span>
                    <span className="text-white/60 text-sm">{tm('package3daysDesc')}</span>
                    <span className="mt-3 text-2xl font-black text-[#E8927C]">{tm('price3days')}</span>
                    <span className="text-xs text-white/40">{tm('perDay')}</span>
                  </button>
                  ...
                  <button
                    ...
                    className="relative flex flex-col items-center gap-2 rounded-2xl bg-white/10 hover:bg-white/20 border-2 border-white/20 hover:border-[#D4A017] p-6 text-white transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <Package className="h-8 w-8 text-[#D4A017] mb-1" />
                    <span className="font-heading font-black text-2xl uppercase">
                      {tm('package6days')}
                    </span>
                    <span className="text-white/60 text-sm">{tm('package6daysDesc')}</span>
                    <span className="mt-3 text-2xl font-black text-[#D4A017]">{tm('price6days')}</span>
                    <span className="text-xs text-white/40">{tm('perDay')}</span>
