...
import { getOfficeDeliveryDates, getOrderDeadline, getEarliestDeliveryDate } from '@/lib/office-delivery';
...
export default function WeeklyOrderPage() {
  ...
  const MENU_DATES = Object.keys(menuData);
  ...
  const deadlineFormatted = orderDeadline
    ? orderDeadline.toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long' }) +
    ' – 10:00'
    : null;
      ...
      <div id="menu-section" className="relative w-full">
        ...
        <div className="bg-[#1B4332] overflow-x-hidden relative z-10">
          <div className="mx-auto max-w-2xl px-5 py-6 md:py-8 text-center">
            <div
              ref={vanRef}
              className={`transition-all duration-700 ease-out ${vanVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-16'} mb-6`}
            >
              <img
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              {earliestDate && (
                <p className="text-sm md:text-base text-[#ed8788] font-medium">
                  {t('nextDeliveryMenu', { date: earliestDate })}
                </p>
              )}

              <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#E8967A]/20 border border-[#E8967A]/30 px-5 py-2 text-sm text-white shadow-sm">
                <span className="font-bold text-[#E8967A] uppercase tracking-wide">{t('freeDelivery')}</span>
              </div>
            </div>
          </div>
        </div>
